...
void bundleAdjustment (
    ...
    for (const Point3f p:points_3d) {
        ...
        point->setMarginalized ( true ); // g2o 中必须设置 marg 参见第十讲   //由于路标要被边缘化计算，所以设置边缘化属性为true
...
int main ()
{
    ...
    for ( DMatch m:matches )
    {
        ...
        auto d=d1.at<unsigned short>(keypoints_1[m.queryIdx].pt.y,keypoints_1[m.queryIdx].pt.x);
        // ushort dd1 = d1.ptr<unsigned short> (int ( keypoints_1[m.queryIdx].pt.y )) [ int ( keypoints_1[m.queryIdx].pt.x ) ];
        //cout<<"d= "<<d<<endl;
        // cout<<"dd1= "<<dd1<<endl;
        if ( d == 0 )   // bad depth
            continue;
        ...
        pts_3d.push_back ( Point3f ( p1.x*dd, p1.y*dd, dd ) );    //第一帧的相机坐标下的特征点坐标
        pts_2d.push_back ( keypoints_2[m.trainIdx].pt );      //第二帧的像素点
    }
